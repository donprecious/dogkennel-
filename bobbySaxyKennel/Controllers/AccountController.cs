...
namespace bobbySaxyKennel.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //remeber to Remove these Anonymous
        [AllowAnonymous]
        //[Authorize(Roles="Admin")]
        [HttpPost, ValidateAntiForgeryToken]
        public async Task<ActionResult> CreateSeller(Models.ViewModels.SellerVm m)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (new Models.ClassModel.Sellers().Add(user.Id, m.AddressableEmail))
                    {
                        return Json(new { status =200, message = "Seller Created Successfully"});
                    }
                    return Json(new {status = 0, message = "failed to create " });
                    //Create Seller
                }
                AddErrors(result);
            ...
            foreach (ModelState modelState in ViewData.ModelState.Values)
            {
                ...
                {
                    strErr.Add(error.ErrorMessage);

                }
            }
            //  return View(m);
            return Json(new { status = 0, message = "failed to create ", errors = strErr});
        }
        //
